      ...
      match ks:
        case Nil{}:
          do IO<Unit>:
            ...
            bs : List<&2, U32> <- T.Tty.read(50, 64)   # bytes within 50 ms
            loop(p, n, T.Tty.keys(bs))                   # decoded
        case T.Up{} <> rest:
          loop(p, (n + 1 : U32), rest)
        case T.Esc{} <> rest:
          IO.pure(Unit, Unit{})
...
| `Tty.read(ms, max)` | `U32 -> U32 -> IO(List<&2, U32>)` | the bytes stdin has within `ms`, at most `max`; `[]` on a timeout. Waits without blocking the other computations |
...
TLetter{c} <> rest`. An escape split across two reads is not a sequence and
decodes as `Esc` and the bytes; read with a `ms` that lets a sequence land
whole (terminals send it in one write).
...
`{$: "Con", head, tail}`, a pair `io_tup(a, b)`, `io_done(v)`/`io_fail(code)`),
