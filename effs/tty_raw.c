...
// Ctrl-C arrives as byte 3, Enter as 13) or off, restored at exit. A
// no-op when stdin is not a terminal, so a piped program still runs.

#include <termios.h>

static struct termios tty_raw_saved;
static int            tty_raw_live = 0;
